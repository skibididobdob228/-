...
uniform float uAmbient;      // minimum light floor (dimension glow)
...
void main() {
    ...
    float lightLevel = max(max(skyContribution + sun, blockContribution), uAmbient);
    lightLevel = clamp(lightLevel, 0.04, 1.0);
